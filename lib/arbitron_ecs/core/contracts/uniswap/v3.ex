defmodule Arbitron.Uniswap.V3 do
  import Math
  ...
  def generate_tick_state(tick_list) when is_list(tick_list) do
  ...
  def generate_tick_state(tick_map) when is_map(tick_map) do
    tick_list =
      Map.to_list(tick_map)
      |> Enum.map(fn {k, v} -> [k, v] end)
      |> Enum.sort(:asc)

    generate_tick_state(tick_list)
  end
