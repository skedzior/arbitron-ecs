defmodule Arbitron.Uniswap.V2 do
  alias Decimal, as: D
  import Arbitron.Contracts.Utils
  ...
  def calculate_optimal_sando(amount_in, amount_out_min, reserve0, reserve1, zero_for_one, fee \\ 30) do
    IO.inspect({amount_in, amount_out_min, reserve0, reserve1}, label: "calculate_optimal_sando: amount_in, amount_out_min, reserve0, reserve1")
    swap_fee = 10000 - fee
    ...
    try do
      ...
      if worst_r_in == 0 do
        IO.inspect({fourac, sqrt, worst_r_in}, label: "fourac, sqrt, worst_r_in")
        throw(worst_r_in)
      end
