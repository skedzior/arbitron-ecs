defmodule ArbitronWeb.PairsLive do
  ...
  def mount(_params, _session, socket) do
    ...
    pairs =
      ECS.Registry.lookup(Pair)
      |> Enum.map(fn {_, pid} -> ECS.Entity.Agent.get(pid) end)
      |> IO.inspect(label: "keysd")
  ...
  def handle_info(pid, socket) do
    state = ECS.Entity.Agent.get(pid)
    IO.inspect(state, label: "CHAIN LIVE")
    {:noreply, assign(
      socket,
      state: state,
      blocks: state.blocks,
